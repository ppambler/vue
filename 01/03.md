...
> 我们给`obj3`添加了一个虚拟属性`xxx`，也叫属性……每次`set`值的时候，我们都得赋值给一个`xxx`，但是`xxx`是不存在的，`obj3` 是咩有这个属性的，所以我们必须得找一个东西来放置`set`过来的值，所以我们定义了一个叫`_xxx`的东西，用来存储`xxx`的值，每次`get`值，实际拿到的是`_xxx`的值，也就是说`get`和`set`与`_xxx`构成了闭包……
