...
### <mark>2）用 TS 类 `<script lang="ts">`</mark>
...
### <mark>3）用 JS 类 `<script lang="js">`</mark>
...
为啥要 `dataSource` 要加 `readonly`？

![readonly](assets/img/2020-09-02-23-28-01.png)

上边的代码 `tsc` 并没有提示爆红，因为我们是直接追加元素，并没有更改`dataSource` 的地址，而 `tsc` 功能有限，所以就没有提示爆红了 -> 如果你直接这样 `this.dataSource = []`，那么就会报错——「你不能把一个值赋值给一个 `const` or `readonly` 的变量」
